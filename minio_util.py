...
BUCKET_NAME = "real-estate-properties"
...
async def upload_json(zip: str, data):
    async with semaphore:
        # Parse the json file name
        file_name = f'{datetime.now().strftime("%Y-%m-%d")}/{zip}.json'

        # Convert data dict to stream bytes
        content = "\n".join(data) + "\n"
        data_bytes = BytesIO(content.encode("utf-8"))
